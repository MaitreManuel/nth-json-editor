...
export const ObjectComponent = {
  support: (value: unknown) => typeof value === 'object' && value !== null && !Array.isArray(value),
    ...
    return `
      <div class="json__node">
        ...
        <div class="json__node--value">
          ${this.renderChildren(value as Record<string, unknown>, registry, `${parentPath ? `${parentPath}.`: ''}${key}`)}
        </div>
      </div>
    `;
  }
